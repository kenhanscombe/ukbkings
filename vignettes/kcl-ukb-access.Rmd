...
If you have any trouble installing the package or its dependencies, create a conda environment from **ukbkings-env.yml**. From bash,
...
# run R, install ukbkings, inspect/ retrieve UKB data (see below)
source deactivate ukbkings-env
```

The environment created, `ukbkings-env`, includes R 3.6, the `ukbkings` package, and all base and essential R packages ([based on this setup](https://docs.anaconda.com/anaconda/user-guide/tasks/using-r-language/)), as well as Python 3.7 and Snakemake 5.7. Once created use `source activate` to start it up, and `source deactivate` to tear it down.
...
You will need to create a file in your user directory that contains the fields you require, one per line. You can use a pager to manually search through the available fields.
...
Or, as `proj_fields` is a dataframe, you can use regular expressions to search the `field` or `name` columns. For all *index* and *array* entries of a particular field

```{r eval=TRUE}
# Search `field`
proj_fields %>%
  select(field, name) %>%
  filter(str_detect(field, "^21000-.*"))
```

<br>

Write to file with `bio_field_add` (creates file if it does not exist, or appends to existing file)

```{r}
proj_fields %>%
  filter(str_detect(field, "^21000-.*")) %>% 
  bio_field_add("<field_subset_file.txt>")
```

**Note**: No need to add `eid` (the unique sample identifier) - it is automatically added when retrieving fields with `bio_phen`.
...
proj_fields %>%
  select(field, name) %>%
  filter(str_detect(name, "assessment_centre"))
```
...
Write a dataframe of selected study fields to you user directory.

```{r}
bio_phen(proj_dir, fields = "<selected_study_fields_file>")
```

<br>

`bio_phen` uses `saveRDS` to write a serialized version of your data. Use `readRDS` to "resurrect" your serialized data as a dataframe.

```{r}
df <- readRDS("<study_data.rds>")
```
