...
def link_genetics(fam=None, sample=None, rel=None, initialized=False):
    ...
    Args:
        fam (str): fam file name
        sample (str): sample file name
        rel (str): relatedness file name
    """
    p = Path('.')
    wd = p.absolute()
    raw = wd / 'raw'
    ...
    if rel:
        try:
            os.system(f'ln -s {raw}/{rel} imputed/')
        except FileExistsError:
            print(
