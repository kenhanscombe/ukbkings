...
proj_fields %>%
  filter(str_detect(field, "^21000-.*")) %>% 
  pull(field) %>% 
  cat(file = "<selected_study_fields>", sep = "\n")
```

**Note**: No need to add `eid` (the unique sample identifier) - it is automatically added when retrieving fields with `bio_data`.
...
proj_fields %>%
  filter(str_detect(name, "assessment_centre")) %>%
  pull(field) %>%
  cat(file = "<selected_study_fields_file>", sep = "\n", append = TRUE)
```
...
```{r}
bio_data(proj_dir, fields = "<selected_study_fields_file>")
```

<br>

`bio_data` uses `saveRDS` to write a serialized version of your data. Use `readRDS` to "resurrect" your serialized data.
