...
[Access UKB data on Rosalind](https://kenhanscombe.github.io/ukbkings/articles/kcl-ukb-access.html#record-level-data)
...
resources folder on Rosalind. (See the
...
The drug_name and quantity columns are messy, with 1,655 and 4,830
unique entries in this subset of prescriptions.

```{r}
px %>% distinct(drug_name)
```
...
# A tibble: 1,655 × 1
 ...
 2 Citalopram 10mg tablets                
 3 Citalopram 20mg tablets                
 4 Citalopram 40mg tablets                
 5 Sertraline 50mg tablets                
 6 Amitriptyline 10mg tablets             
 7 Escitalopram 5mg tablets               
 8 Escitalopram 20mg tablets              
 9 Escitalopram 10mg tablets              
10 Mirtazapine 15mg orodispersible tablets
# … with 1,645 more rows
</pre>
</span>

```{r}
px %>% distinct(quantity)
```
...
Tidy up drug_name and quantity.
```{r}
tidy_rgx <- "[\\*\\(\\)\\-\\/\\[\\]\\&\\'\\,]|\\.{3}"
